...
function run(raw: string) {
	...
	switch (name) {
		case "help":
			out("help  ls [-a]  cd <dir>  cat <file>  whoami  neofetch  clear  exit");
			break;
		case "ls": {
		...
		case "cd": {
			...
			if (target === "~" || target === "/" || target === "") {
				...
				["projects", "photos", "experience", "about", "contact"].includes(
					target,
				)
			) {
				onclose();
	...
	<div class="mx-auto max-w-4xl px-6 py-4">
		<div bind:this={scrollEl} class="max-h-[40svh] overflow-y-auto">
			{#each lines as line, i (i)}
				<p class="whitespace-pre-wrap" class:text-muted={line.kind === 'in'}>{line.text}</p>
			{/each}
		</div>
