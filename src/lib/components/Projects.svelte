...
{#if projects.length > 0}
	<section class="mx-auto max-w-4xl px-6 py-24">
		<h2 id="projects" class="scroll-mt-8 font-mono text-sm text-muted">
			projects
		</h2>
		...
		<ul class="mt-8 divide-y divide-line border-y border-line">
			{#each projects as project (project.slug)}
				...
				<li>
					<div
						class="group grid gap-1 py-5 sm:grid-cols-[10rem_1fr_auto] sm:gap-6"
					>
						...
						{#if project.meta.year}
							<span class="font-mono text-xs text-muted sm:pt-1"
								>{project.meta.year}</span
							>
						{/if}
					</div>
				</li>
